                ...
                Select Case LineData
                    Case "CDCD", "CDCP"
                        ...
                        If CalculateType.InstallStatus = True Then
                            If FileIO.FileSystem.DirectoryExists(FileIO.FileSystem.CombinePath(FileIO.FileSystem.CombinePath(GamePath, "Mods"), x)) = True Then
...
PR1:
                        ...
                        ManifestFile.SearchManifests(FileIO.FileSystem.CombinePath(ItemPath, x))
                        ...
                        For ManifestFileList = 0 To ManifestFile.FileCollection.Length - 1
                            Dim a As String = FileIO.FileSystem.ReadAllText(FileIO.FileSystem.CombinePath(FileIO.FileSystem.CombinePath(ItemPath, x), ManifestFile.FileCollection(ManifestFileList)))
...
LineVersion:
                            If CalculateType.InstalledVersion = True Then
                                Dim p1 As String = FileIO.FileSystem.CombinePath(GamePath, "Mods")
                                Dim p2 As String = FileIO.FileSystem.CombinePath(p1, x)
                                Dim p3 As String = FileIO.FileSystem.CombinePath(p2, ManifestFile.FileCollection(ManifestFileList))
                                If FileIO.FileSystem.FileExists(p3) = True Then
                                    Dim b As String = FileIO.FileSystem.ReadAllText(p3)
                            ...
                            If CalculateType.UpdateKeys = True Then
                                ...
                                For k = 0 To JsonData.item("UpdateKeys").Count - 1
                                    If InStr(JsonData.item("UpdateKeys").item(k).ToString.ToLower, "nexus") > 0 Then
                                        Dim str = SharedFunction.GetModUpdateID(JsonData.item("UpdateKeys").item(k).ToString.ToLower, "nexus")
                                    ...
                                    ElseIf InStr(JsonData.item("UpdateKeys").item(k).ToString.ToLower, "moddrop") > 0 Then
                                        Dim str = SharedFunction.GetModUpdatePlatformAddress(JsonData.item("UpdateKeys").item(k).ToString, "moddrop")
                                        If str <> "" Then
                                    ...
                                    ElseIf InStr(JsonData.item("UpdateKeys").item(k).ToString.ToLower, "github") > 0 Then
                                        Dim str = SharedFunction.GetModUpdatePlatformAddress(JsonData.item("UpdateKeys").item(k).ToString, "github")
                                        If str <> "" Then
                                    ...
                                    ElseIf InStr(JsonData.item("UpdateKeys").item(k).ToString.ToLower, "chucklefish") > 0 Then
                                        Dim str = SharedFunction.GetModUpdateID(JsonData.item("UpdateKeys").item(k).ToString.ToLower, "chucklefish")
                                        If IsNumeric(str) Then
...
LineEnd:
                    Case "CDGCD"
                        If CalculateType.InstallStatus = True Then
                            ...
                            Select Case InstallStatus
                                Case 1, 2, 3
                                Case -1
                                    If FileIO.FileSystem.DirectoryExists(FileIO.FileSystem.CombinePath(GamePath, x2)) = True Then
                    ...
                    Case "CDGRF"
                        If CalculateType.InstallStatus = True Then
                            ...
                            Dim x2 As String = line(i + 2)
                            i += 2
                            If FileIO.FileSystem.FileExists(FileIO.FileSystem.CombinePath(ItemPath, x1)) = False Then
                                InstallStatus = Objects.InstallStatus.FileNotReplaced
                                Continue For
                            End If
                            If FileIO.FileSystem.FileExists(FileIO.FileSystem.CombinePath(GamePath, x2)) = False Then
                                InstallStatus = Objects.InstallStatus.FileNotReplaced
                                Continue For
                            End If
